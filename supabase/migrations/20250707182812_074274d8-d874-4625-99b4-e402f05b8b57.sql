-- Fix the trainer_athletes status to use 'pending' which is the default
...
WHERE EXISTS (SELECT 1 FROM profiles WHERE role = 'trainer') 
  ...
  AND NOT EXISTS (SELECT 1 FROM trainer_athletes);
...
WHERE NOT EXISTS (SELECT 1 FROM workout_schedules)
LIMIT 1;
...
SELECT 
  ...
  'completed'
FROM trainer_athletes ta
WHERE (SELECT COUNT(*) FROM workout_schedules) < 2
LIMIT 1;
...
SELECT 
  ...
  'completed'
FROM trainer_athletes ta
WHERE (SELECT COUNT(*) FROM workout_schedules) < 3
LIMIT 1;